...
function flipNote(element) {
    if (element.classList.contains('flipping')) return; // Prevents glitching if clicked twice
    
    element.classList.add('flipping');
    
    // Changing this from 150 to 300 stops the card from disappearing
    setTimeout(() => {
...
async function submitQuiz() {
    ...
    for(let i = 1; i <= 2; i++) { 
        ...
        if (selectElement) { score += parseInt(selectElement.value); }
    }
    let advice = "";
